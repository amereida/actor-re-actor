...
int MINIMA_DISTANCIAcm = 50;   // minima distancia para medir
int MAXIMA_DISTANCIAcm = 400;  // máxima distancia para medir
int NUM_MAX_PASOS = 10000; // <---- DEFINIR AL COMIENZO DEL MONTAJE, es el número máximo de pasos entre una posicion inicial y una final
int x = 0; // manual target

////////////////////// VARIABLES FIJAS DE CALCULO /////////////////////////////
int PASOS_DE_SEGURIDAD = 40;  // holgura de pasos para la cantidad minima de pasos y la maxima
...
int DIF_MIN_MEDIDAS = 30; // variacion minima que debe existir entre dos medidas consecutivas para que actue el motor
...
void loop(){                                   
  ...
  if (modo == 0){ 
   ...
   if(cm >= MINIMA_DISTANCIAcm && cm <= MAXIMA_DISTANCIAcm){   
     
     //si la diferencia entre los ultimas 2 lecturas es mayor a DIF_MIN_MEDIDAS mostrarra cambio en motor y luz
     if(abs(cm_anterior - cm) >= DIF_MIN_MEDIDAS){
          unsigned long CERO = 0; 
          ...
          PASOS = map(cm,MINIMA_DISTANCIAcm,MAXIMA_DISTANCIAcm,MAX_PASOS_SEGUR,MIN_PASOS_SEGUR); 
          unsigned long tiempo = millis();
          
          stepper.runToNewPosition(PASOS);
          x = PASOS; // igualo esta variable para el modo manual      
        }else if(cm <= MAXIMA_DISTANCIAcm + 20 && cm >= MAXIMA_DISTANCIAcm - 20 || cm <= MAXIMA_DISTANCIAcm + 20 && cm >= MAXIMA_DISTANCIAcm - 20){
              dimtime = 6750;
              unsigned long CERO = 0; 
              unsigned long tiempo = millis();
