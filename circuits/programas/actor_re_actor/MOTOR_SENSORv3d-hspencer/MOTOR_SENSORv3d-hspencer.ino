...
int MINIMA_DISTANCIAcm = 10;   // minima distancia para medir
int MAXIMA_DISTANCIAcm = 450;  // máxima distancia para medir
...
void BOTON_MANUAL(){  
  ...
  stepper.setMaxSpeed(6000.0); // pasos por segundo
  stepper.setSpeed(5000.0);     // pasos por segundo
  delayMicroseconds(50);
...
void setup(){  
  ...
  Serial.begin(9600);
  stepper.setMaxSpeed(3000.0);  // LIB ACCEL
  stepper.setAcceleration(1000.0);  // LIB ACCEL    
  pinMode(reloj,INPUT);
...
void loop(){                                   
  ...
  if (modo == 0){ 
   ...
   if(MINIMA_DISTANCIAcm <= cm && MAXIMA_DISTANCIAcm >= cm)  
    {     
